...
def register(request):
    if request.method == "POST":
        ...
        try:
            user = User.objects.create_user(username, email, password,first_name=fname,last_name=lname,house=house,street=street,city=city,state=state,phone=phone)
            user.save()
...
def listing(request, listing_id):
    is_listed_user=False
    user = request.user
    listing = AuctionListing.objects.filter(id=listing_id).first()
    listed_user=User.objects.get(id=listing.user_id)
    if(user.id==listed_user.id):
        is_listed_user=True
    print(is_listed_user)
...
def reply(request,comment_id,listing_id):
    if request.method == 'POST':
